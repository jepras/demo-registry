...
- **Writes** (adding a demo) go through one tiny Cloudflare Worker that holds the only credential. Both the web form and the Claude skill use it. No GitHub account needed to submit.
- **Nothing can be deleted:** the Worker only appends, a CI check rejects any removal/edit, and git history preserves every version.
...
| `.github/workflows/append-only.yml` | CI: fails if any entry is removed or modified |
...
## Adding demos
- **Anyone:** open the Pages site → **+ Add a demo** → fill the form.
